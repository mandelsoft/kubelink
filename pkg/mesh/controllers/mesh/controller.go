...
func init() {
	...
	controller.Configure(NAME).
		...
		MainResourceByGK(api.MESH).
		Reconciler(Create).
		With(reconcilers.SlaveReconcilerForGKs("ipam", controller.CLUSTER_MAIN, ipamapi.IPAMRANGE)).
		With(reconcilers.UsageReconcilerForGKs("usage", controller.CLUSTER_MAIN, controllers.SECRET)).
		MustRegister()
}
...
func Create(c controller.Interface) (reconcile.Interface, error) {
	...
	base, err := controllers.NewReconcilerWithSlave(c, ipamapi.IPAMRANGE, this.cleanupHandler)
	if err != nil {
		return nil, err
	}
	this.database = database.GetDatabase(c)
	this.ReconcilerWithSlaves = base
	this.resc, _ = c.GetMainCluster().GetResource(api.MESH)
	return this, nil
}
